...
function formatType(type) {
  ...
  switch (type.type) {
    ...
    case 'union':     return type.types.map(formatType).join(' \\| ');
    case 'reference':
      if (type.name === 'Partial') return '`object`';
      if (type.name === 'Direction') return '`"up"` \\| `"down"` \\| `"left"` \\| `"right"`';
      return `\`${type.name}\``;
    case 'reflection':
      return '`function`';
...
// labelGroups
lines.push('| `labelGroups` | `object[]` | An array of label groups that define how to resolve overlaps. |');
lines.push('| `labelGroups[].technique` | `"nudge" \\| "choices" \\| "fixed"` | The overlap avoidance technique to use. `nudge` shifts labels by a small offset; `choices` picks from a list of candidate positions; `fixed` treats nodes as immovable obstacles. |');
...
const optionsOrder = ['includeParent', 'parentMargin', 'iterations', 'temperature', 'coolingRate', 'scoreExponent', 'seed', 'debug'];
const optionsProps = Object.fromEntries(
  getOwnProps('Options').map((p) => [p.name, p])
);
for (const name of optionsOrder) {
  if (optionsProps[name]) lines.push(renderProp(`options.${name}`, optionsProps[name]));
}
...
if (startIdx === -1 || endIdx === -1) {
  console.error('Could not find <!-- params-start --> / <!-- params-end --> markers in README.md');
  process.exit(1);
}
