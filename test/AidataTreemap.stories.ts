...
function getArrowConfig(direction: string, chartWidth: number, chartHeight: number): ArrowConfig {
  const s = SWOOP;
  const p = SWOOP_PAD;
  switch (direction) {
    case 'top-left':
      return {
        ...
        anchorY: (d) => d.y0,
        path: `M ${-s} ${-s-p-1} L ${-s+9} ${-s-p-1} Q 0 ${-s-p-1} 0 ${-s+9} L 0 ${-p-1} L 0 ${-p}`,
        textOffset: [-s - p, -s],
        textAnchor: 'end',
      };
    case 'top-right':
...
function pickDirection(d: any, chartWidth: number, chartHeight: number): string {
  ...
  if (chartWidth - d.x1 < 100) return d.y0 - 100 < 0 ? 'bottom-left' : 'top-left';
...
function buildTreemap(
  ...
  technique: 'choices' | 'annealing' = 'choices'
) {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      const makeChoice = (dir: string, d: any) => (node: Element) => {
        const cfg = getArrowConfig(dir, chartWidth, chartHeight);
        node.setAttribute('transform', `translate(${cfg.anchorX(d)},${cfg.anchorY(d)})`);
      ...
      const ALL_DIRECTIONS = [
        ...
        'middle-right', 'middle-left', 'middle-up', 'middle-down',
      ];
      ...
      const choicesGroups: LabelGroup[] = (nodeDomainText.nodes() as Element[]).map((node, i) => {
        ...
        const ordered = [preferred, ...ALL_DIRECTIONS.filter((dir) => dir !== preferred)];
        return {
          technique,
          nodes: [node],
      ...
      const nudgeRender = (node: Element, dx: number, dy: number) => {
        const m = (node.getAttribute('transform') || 'translate(0,0)')
          .match(/translate\(([^,]+),\s*([^)]+)\)/);
...
export const ManySitesAnnealing: StoryObj = {
  ...storyDefaults,
  play: async ({ canvasElement }) => {
    const div = canvasElement.querySelector('div') as HTMLElement;
    div.innerHTML = '';
    buildTreemap(div, ACTIVE_DOMAINS_MANY, 'annealing');
  },
};
