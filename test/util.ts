...
import type { LabelGroup, ScoredOptions } from '../src/index';
...
export const playScored = async ({ canvasElement, args }) => {
  const parent = canvasElement.querySelector('svg');
  const parentBounds = parent.getBoundingClientRect();
  ...
  const scoredOptions: ScoredOptions = { ...args.options, ...args.scoredOptions };
  const avoidOverlap = new AvoidOverlap();
  avoidOverlap.runScored(parent, labelGroups, scoredOptions);
};
